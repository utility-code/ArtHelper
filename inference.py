...
import numpy as np
import torchsnooper
# Grad cam part (https://github.com/jacobgil/pytorch-grad-cam/blob/master/gradcam.py)
...
@torchsnooper.snoop()
def grad_pred(test_path,inp_path,cl):
    img = cv2.imread(test_path+inp_path, 1)
    cv2.imwrite(f'./outputs/orig_{inp_path.split(".")[0]}.jpg', img)
    ...
    cv2.imwrite(f'./outputs/{cl}_{inp_path.split(".")[0]}.jpg', cam_gb)
